...
This skill orchestrates three others: an inline mining pass (see step 1), a local skill-authoring flow (write SKILL.md under the project or plugin skills/) (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.omp/skills/**/*-mode/SKILL.md` and `~/.omp/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.omp/skills/<handle>/`), not only at the top level. If one exists, confirm intent with OMP `ask` (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's `agent-transcripts/` directory. Use only that path. Don't glob across `~/.omp/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Use the OMP `ask` tool (structured multi-choice) rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.
...
- a local skill-authoring flow (write SKILL.md under the project or plugin skills/) skill: skill authoring process and writing guidelines.
